...
function playerMove() {
  var playerelement = LoadingPlayer("query");
  var textures = [];
  var animation = [];
  ...
  for (let index = 0; index < playerelement.length; index++) {
    ...
    for (var i = 0; i < Map.children.length; i++) {
      for (var a = 0; a < Map.children[i].children.length; a++) {
        if (Map.children[i].children[a].getAttribute("tag")) {
          var tag = JSON.parse(Map.children[i].children[a].getAttribute("tag"));
          if (!tag["type"]) {
          ...
          if (type == "wall") {
            var A = checkCrash(Map.children[i].children[a]);
            var B = checkCrash(playerelement[index]);
            var collision = checkCrash(
              Map.children[i].children[a],
              playerelement[index]
            );
            if (collision == true) {
              var number = 0.15;
    ...
    if (Operation["MoveTop"] >= 1) {
      ...
      playerelement[index].style.top = number + "%";
    }
    if (Operation["MoveLeft"] >= 1) {
      ...
      playerelement[index].style.left = number + "%";
    }
